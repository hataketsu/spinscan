...
object Updater {
    ...
    fun install(ctx: Context, info: Info, cb: Listener) {
        try {
            ...
            cb.status("Đang tải ${info.versionName}…")
            ...
            if (info.sha256.isNotEmpty()) {
                cb.status("Đang đối chiếu SHA-256…")
                if (!sha256(apk).equals(info.sha256, ignoreCase = true)) {
                    apk.delete()
                    cb.done(false, "SHA-256 không khớp — đã bỏ bản tải về")
                    return
                }
            }

            cb.status("Bàn giao cho trình cài đặt hệ thống…")
            commit(ctx, apk)
            cb.done(true, "Xác nhận cài đặt trên hộp thoại hệ thống")
        } catch (e: Exception) {
